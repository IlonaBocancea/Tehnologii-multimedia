...
document.addEventListener("DOMContentLoaded", function () {
    ...
    ev.addEventListener('mousemove',function (e:any) {
        if (e.target['id'] !== 'svgid'){
            current = document.querySelector('.elm');
            current.classList.remove("elm");
            e.target.classList.add("elm");
        }
    })

    ev.addEventListener('click',function (e:any) {
        if (e.target['id'] !== 'svgid'){
            let x = e.target['id'] ;
            console.log(obj[x]);
        }
